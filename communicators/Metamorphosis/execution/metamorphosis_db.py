...
class transponder_internal:
    ...
    def handle_connection(self, conn):
        ...
        """
        manifest.info("New connection established", process_path="Metamorphosis.DB.metamorphosis_db.imports!.tier2.transponder_internal.handle_connection")

        try:
            while True:
                data = self.recv(conn, 4096)
                if not data:
                    manifest.error("Connection closed by remote side", process_path="Metamorphosis.DB.metamorphosis_db.imports!.tier2.transponder_internal.handle_connection")
                    break

                manifest.info(f"Received: {data}", process_path="Metamorphosis.DB.metamorphosis_db.imports!.tier2.transponder_internal.handle_connection")
            ...
            manifest.error(f"Connection error: {e}", process_path="Metamorphosis.DB.metamorphosis_db.imports!.tier2.transponder_internal.handle_connection")
...
class transponder:
    ...
    def persistent_server(host, port):
        ...
        manifest.info("Transponder active", process_path="Metamorphosis.DB.metamorphosis_db.imports!.tier2.transponder.persistent_server")
